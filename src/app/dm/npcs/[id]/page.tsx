...
import { ArrowLeft, Save, Plus, Trash2, Sword, Shield, Zap, EyeOff } from 'lucide-react';
...
export default function EditNPCPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  async function fetchNPC() {
    try {
      ...
      if (!res.ok) {
        setError(data.error || 'Failed to load NPC');
        return;
      }
      ...
      setChallengeRating(npc.challengeRating || '');
      setWorldId(npc.worldId || '');
      setDefaultHideHp(npc.defaultHideHp ?? true);

      // Load location data
      if (npc.locationResourceId) {
        setLocationResourceId(npc.locationResourceId);
        setUseCustomLocation(false);
      } else if (npc.location) {
        setCustomLocation(npc.location);
      ...
      if (npc.stats) {
        setStats({
          hp: npc.stats.hp || 10,
          ac: npc.stats.ac || 10,
          str: npc.stats.str || 10,
          dex: npc.stats.dex || 10,
          con: npc.stats.con || 10,
          int: npc.stats.int || 10,
          wis: npc.stats.wis || 10,
          cha: npc.stats.cha || 10,
          speed: npc.stats.speed || 30,
        });

        if (npc.stats.attacks) {
          setAttacks(npc.stats.attacks.map((a: Omit<Attack, 'id'>) => ({
            ...a,
  ...
  async function handleSave() {
    ...
    try {
      ...
      if (!res.ok) {
        setError(data.error || 'Failed to save NPC');
        return;
      }

      setSuccessMessage('NPC saved successfully!');
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900">
      <div className="container mx-auto px-4 py-8 max-w-3xl">
        ...
        <div className="flex items-center justify-between mb-8">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-3xl font-bold text-gray-900 dark:text-white">Edit NPC</h1>
              <p className="text-gray-600 dark:text-gray-400">Modify NPC details</p>
            </div>
          </div>
      ...
      {/* AI Assistant */}
      <AIAssistant
        context="I'm editing a D&D 5e NPC (Non-Player Character). Help me with personality, backstory, dialogue, motivations, and secrets."
        quickPrompts={NPC_QUICK_PROMPTS}
        title="NPC Builder AI"
        placeholder="Ask about NPC personality, backstory, dialogue..."
        emptyStateText="Need help with your NPC? Ask me!"
      />
    </div>
  );
}
